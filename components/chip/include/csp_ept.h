...
*/
 ...
 typedef struct
 {
   ...
   __IOM uint32_t	AQCR1;      //0x0048	Action qualify of pwm1
   __IOM uint32_t	AQCR2;    	//0x004C	Action qualify of pwm2
   __IOM uint32_t	AQCR3; 		//0x0050	Action qualify of pwm3
   __IOM uint32_t	AQCR4;  	//0x0054	Action qualify of pwm4
...
static inline void csp_ept_dbg_enable(csp_ept_t *ptEptBase, bool bEnable)
{
...
static inline void csp_ept_set_tinsel(csp_ept_t *ptEptBase, csp_ept_tin_e eVal)
{
	ptEptBase -> CEDR = (ptEptBase -> CEDR & (~EPT_TIN_MSK)) | (eVal << EPT_TIN_POS);
}
...
static inline void csp_ept_set_evps(csp_ept_t *ptEptBase, uint32_t wVal)
{
	ptEptBase ->  EVPS = wVal;
}

static inline void csp_ept_set_trgftcr(csp_ept_t *ptEptBase, uint32_t wVal)
{
	ptEptBase ->  TRGFTCR = wVal;
}
static inline void csp_ept_set_trgftwr(csp_ept_t *ptEptBase, uint32_t wVal)
{
	ptEptBase ->  TRGFTWR = wVal;
}
